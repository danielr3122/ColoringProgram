...
def paint_screen(path, colorNum):
    ...
    # print("Selecting color: " + str(colorNum + 1))
    pyautogui.moveTo(215, 1370)
    pyautogui.click()
...
def is_finished():
    if pyautogui.pixel(215, 1425) != border_color:
        return False
    return True
...
def paintImage(colorNum):
    width, height = find_dimensions()
    if not is_finished():
        for k in range(colorNum):
            if os.path.exists(dirPath + str(k + 1) + ".png"):
                panToStart()
                paint_screen(dirPath + str(k + 1) + ".png", k)
                paint_screen(dirPath + str(k + 1) + ".png", k)
                ...
                for i in range(width):
                    for j in range(height):
                        ...
                        paint_screen(dirPath + str(k + 1) + ".png", k)
                        paint_screen(dirPath + str(k + 1) + ".png", k)
                    if not check_border(1275, 0, 35, 1200):
                        pan_left()
                        if isPanDown:
                            pan_up_half()
                        else:
